...
class FrequenciesDetector:
    ...
    def __init__(self, selected_channel: int, sampling_rate: int, delta_band: list = [0.5, 4], delta_th: float = 20,
                 beta_band: list = [17, 30], beta_th: float = 2, beta_change_th: float = 1, buffer_size: int = 4,
                 delta_decoder_on: bool = True, beta_decoder_on: bool = True):
        ...
        self.buffer_eeg_f = CircularBuffer(1, self.buffer_size_samples)
        ...
        self.beta_change_th = beta_change_th
        self.beta = [100000000000000000000000.0]
        self.beta_prev = [100000000000000000000000.0]
        self.beta_on = True
        self.beta_decoder_on = beta_decoder_on
        self.betad_status = np.zeros((7, 1))
        self.b_bp, self.a_bp = signal.butter(4, np.array([0.5, 45]) * 2 / sampling_rate, 'bandpass')
    ...
    def get_deltad_status(self):
        ...
        self.deltad_status[1, 0] = self.delta_th / 1000
    ...
    def get_betad_status(self):
        ...
        self.betad_status[6, 0] = self.beta_change_th
    ...
    def process(self, seq: int, eeg_signal: np.ndarray) -> (int, np.ndarray, int, bool):
        ...
        eeg_signal_filt, self.zi = signal.lfilter(self.b_bp, self.a_bp, eeg_signal, zi=self.zi)
        eeg_signal = eeg_signal[self.ch, :]
        eeg_signal_f = eeg_signal_filt[self.ch, :]
        eeg_signal = np.reshape(eeg_signal, (1, eeg_signal.shape[0]))
        eeg_signal_f = np.reshape(eeg_signal_f, (1, eeg_signal_f.shape[0]))
        ...
        if (self.previous_seq != -1) and (seq_num_diff > 1):  # connection issues
            ...
            self.buffer_eeg_f.add(np.ones((self.num_channels, int(missing_samples))) * np.nan)
        ...
        self.buffer_eeg_f.add(eeg_signal_f)
        ...
        if data_loss == 0:  # classification can be done only if there has been no data loss
            ...
            if self.blocks_counter >= self.update_blocks:
                ...
                psd_beta, freq = compute_welch_psd(eeg=self.buffer_eeg_f.buffer[:, -self.srate:], window_type='hamming',
                                              window_size=self.srate, noverlap=int(self.srate / 2),
                                              nfft=4 * self.srate, sampling_rate=self.srate, channels=[0],
                                              max_freq=45, log10=False)
                self.delta_activity_check(psd, freq)
                ok_eeg_for_beta = False
                if np.all(np.abs(self.buffer_eeg_f.buffer[:, -self.srate:]) < 200):
                    ok_eeg_for_beta = True
                self.beta_activity_check(psd_beta, freq, ok_eeg_for_beta)
    ...
    def beta_activity_check(self, psd, freq, ok_eeg_for_beta):
        ...
        beta_diff = self.beta - self.beta_prev

        if self.beta < self.beta_th and beta_diff < self.beta_change_th and self.beta_decoder_on:
            self.beta_on = True
